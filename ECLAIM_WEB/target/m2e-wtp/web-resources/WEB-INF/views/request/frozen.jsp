...
<script>
	...
	function createNewRequest() {
		document.location.href = "<%= request.getContextPath() %>/request/frozen";
	}

	function validateForm() {
		var errMsg = "";
	
		if ($("#tblPositionFrozen tbody tr td").html() == "No record found." || $("#tblPositionFrozen tbody tr").size() == 0) {
			errMsg += "Please select at least one post.<br/>";
		}
		
		// Check for all forzen end date
		for (var i=0; i<$("input[name='frozenEndDate']").size(); i++) {
			if ($($("input[name='frozenEndDate']")[i]).val() == "") {
			 	errMsg += "<b>Frozen End Date</b> cannot be empty.<br/>";
				break;
		 	}
		 	else {
		 		if (getDaysBetween($($("input[name='frozenEndDate']")[i]).val(), $("#effectiveDate").val()) < 0) {
		 			errMsg += "<b>Frozen End Date</b> should be on or after Effective Date of Frozen.<br/>";
					break;
		 		} 
		 	}
		}
	...
	function selectPost(clusterCode, instCode, postUid, postId, rankDesc, annualPlanDesc, postDurationDesc, postFte, deptCode, staffGroupCode) {
		...
		for (var i=0; i<$("#tblPositionFrozen tbody tr").length; i++) {
			if ($($($("#tblPositionFrozen tbody tr")[i]).find("td")[0]).text().trim() == postId) {
		...
		$.ajax({
            ...
            success: function(data) {
            	...
            	if (!haveInProgress) {
					...
					if ($("#tblPositionFrozen tbody tr td").html() == "No record found.") {
			        	$("#tblPositionFrozen tbody").remove();
			        }
					...
					row += "<td>" + postFte + "</td>";
					row += "<td><div class='input-group date'><input type='text' name='frozenEndDate' class='form-control'><span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span></div></td>";
					...
					row += "</tr></tbody>";
					if ($("#tblPositionFrozen tbody").length == 0) {
						$("#tblPositionFrozen thead").after(row);
					}
					else {
						$("#tblPositionFrozen tbody:last").after(row);
					}
					...
					// Initial the date picker
				  	...
				  	$("input[name='frozenEndDate']").change(function(e) {
	...
	$(function(){
		...
		if (status != "NEW" && status != "") {
			...
			$("input[name='btnRemove']").prop("disabled", true);
		}
		
		if ($("#tblPositionFrozen tbody tr").length == 0) {
			$("#tblPositionFrozen thead").after("<tbody><tr><td colspan='7'>No record found.</td></tr></tbody>");
		}
	...
	function enableEffectiveDate() {
		if ($("#tblPositionFrozen tbody tr").size() > 0 && $("#tblPositionFrozen tbody tr td").html() != "No record found.") {
...
<div id="page-content-wrapper">
	<div class="container-fluid">
		<div class="breadcrumbs">
			<a href="<c:url value="/home/home"/>"><i class="fa fa-home"></i>Home</a> > Request > Frozen 
		</div>
		...
		<div class="section-title">
			<div class="title pull-left"><i class="fa fa-file-text-o"></i>Frozen Post</div>
		</div>
		<form id="frmDetail" method="POST" enctype="multipart/form-data">
				...
				<div class="panel-body">
					...
					<div class="row">
						<div class="form-group">
						  	<div class="col-sm-2">
							  	<label for="request_id" class="field_request_label">Requester<font class='star'>*</font></label>
							</div>
							<div class="col-sm-4">
						...
						<div class="form-group">
						  	<div class="col-sm-3">
							  	<label for="request_status" class="field_request_label">Effective Date of Frozen<font class='star'>*</font></label>
							</div>
							<div class="col-sm-3">
			...
			<div class="panel panel-custom-primary">
				<div class="panel-heading">
					<div class="panel-heading-title">Post to be Frozen</div>
				</div>
				<div class="panel-body" style="padding-top:5px">
					<table id="tblPositionFrozen" class="table-bordered mprs_table">
						<thead>
							<tr>
								<th>Post ID</th>
								<th>Rank</th>
						...
						<tbody>
							<c:forEach var="listValue" items="${formBean.requestPositionList}">
								<tr>
									...
									<td><div class="input-group date"><input type="text" name="frozenEndDate" class="form-control" value='<fmt:formatDate value="${listValue.postStatusEndDate}" pattern="dd/MM/yyyy"/>'><span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span></div></td>
					...
					<div class="row">
						<div class="col-sm-2">
							<label for="txt_reason" class="field_request_label"><strong>Reason of frozen</strong></label>
						</div>
						<div class="col-sm-9">
