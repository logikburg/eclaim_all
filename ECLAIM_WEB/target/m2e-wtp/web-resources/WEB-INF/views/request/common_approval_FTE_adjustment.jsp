...
<script>
	...
	function editEmail() {
		...
		if ($("#hiddenFormAction").val() == "SUBMIT") {
			...
			else if ($("#requestType").val() == "FTE_ADJ") {
				emailTemplateId = "RQ_FTE_ADJ_SUBMIT";
			}
		}
			...
			if ($("#requestType").val() == "NEW") {
				...
				emailTemplateId = "RQ_FTE_ADJ_CONFIRM";
			}
		}
		...
		if ($("#requestType").val() == "FTE_ADJ") {
			for (var i=1; i< $("#tblFteAdjustment tr").length; i++) {
				if (i != 1) {
					postIdList += ", ";
				}
			
				postIdList += $($($("#tblFteAdjustment tr")[i]).find("td:eq(0)")).text();
			}
		}
