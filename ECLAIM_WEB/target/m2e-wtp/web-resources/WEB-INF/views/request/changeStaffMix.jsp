...
<script>
	...
	function editNewPosition(postID, obj, idx){
		...
		// $('#editPostModel #rd_Duartion').val($("#tblPositionTo tbody tr[id='"+postID+"'] input[name='RequestRdDuration']").val());
		...
		$('#editPostModel #txt_limit_duration_end_date').val($("#tblPositionTo tbody tr[id='"+postID+"'] input[name='RequestPostActualEndDate']").val());
		...
		// Reset check status for Limited End Date
		$("#frmDialog").bootstrapValidator('updateStatus', 'limitDurationEndDate', 'NOT_VALIDATED');
	...
	function saveNewPosition(){
		...
		if (!$("#editPostModel #txt_postStartDate").prop("disabled") 
		       && !$("#editPostModel #txt_limit_duration_end_date").prop("disabled")) {
			var postStartDate = $("#editPostModel #txt_postStartDate").val();
			var postEndDate = $("#editPostModel #txt_limit_duration_end_date").val();
		...
		row += "<td><input type='hidden' name='requestPostNoTo' value='" + postSeq + "'/></td>";
		row += "<td>" + rank_desc + "</td>";
		...
		row += "<td style='text-align:center'><button type='button' name='btnEdit' class='btn btn-primary' onclick='editNewPosition(\""
				+ postSeq
				+ "\", this, " + rowNum + ")' style='margin-right:5px;'>Edit</button><button type='button' class='btn btn-primary' onclick='removeToPost($(this));enableEffectiveDate()'><i class='fa fa-times'></i> Remove</button>";
		//Position Details
		...
		row += "<input type='hidden' name='RequestPostActualEndDate' value='" + $("#editPostModel #txt_limit_duration_end_date").val() + "'/>";
		...
		//Funding
		/*row += "<input type='hidden' name='requestAnnualPlanInd' value='" + $("#editPostModel #ddl_annual_plan_ind").val() + "'/>";
		row += "<input type='hidden' name='requestProgramYear' value='" + $("#editPostModel #ddl_program_year").val() + "'/>";
		row += "<input type='hidden' name='requestProgramCode' value='" + $("#editPostModel #txt_program_code").val() + "'/>";
		row += "<input type='hidden' name='requestProgramName' value='" + $("#editPostModel #txt_program_name").val() + "'/>";
		row += "<input type='hidden' name='requestProgramRemark' value='" + $("#editPostModel #txt_program_remark").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc1st' value='" + $("#editPostModel #ddl_1st_funding_source").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc1stStartDate' value='" + $("#editPostModel #txt_fund_src_1st_start_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc1stEndDate' value='" + $("#editPostModel #txt_fund_src_1st_end_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc2nd' value='" + $("#editPostModel #ddl_2nd_funding_source").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc2ndStartDate' value='" + $("#editPostModel #txt_fund_src_2nd_start_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc2ndEndDate' value='" + $("#editPostModel #txt_fund_src_2nd_end_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc3rd' value='" + $("#editPostModel #ddl_3rd_funding_source").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc3rdStartDate' value='" + $("#editPostModel #txt_fund_src_3rd_start_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundSrc3rdEndDate' value='" + $("#editPostModel #txt_fund_src_3rd_end_date").val() + "'/>";
		row += "<input type='hidden' name='requestFundRemark' value='" + $("#editPostModel #fund_remark").val() + "'/>";
		row += "<input type='hidden' name='requestFundCostCode' value='" + $("#editPostModel #txt_fund_cost_code").val() + "'/>";
		
		row += "<input type='hidden' name='requestFund2ndRemark' value='" + $("#editPostModel #fund_src_2nd_remark").val() + "'/>";
		row += "<input type='hidden' name='requestFund3rdRemark' value='" + $("#editPostModel #fund_src_3rd_remark").val() + "'/>";
		row += "<input type='hidden' name='requestFund1stFTE' value='" + $("#editPostModel #fund_src_1st_fte").val() + "'/>";
		row += "<input type='hidden' name='requestFund2ndFTE' value='" + $("#editPostModel #fund_src_2nd_fte").val() + "'/>";
		row += "<input type='hidden' name='requestFund3rdFTE' value='" + $("#editPostModel #fund_src_3rd_fte").val() + "'/>";*/
	...
	function selectPost(clusterCode, instCode, postUid, postId, rankDesc, annualPlanDesc, postDurationDesc, postFte, deptCode, staffGroupCode) {
		...
		if (fromPost == "Y") {
			...
			for (var i=0; i<$("#tblPositionFrom tbody tr").length; i++) {
				if ($($($("#tblPositionFrom tbody tr")[i]).find("td")[0]).text().trim() == postId) {
			...
			$.ajax({
	            ...
	            success: function(data) {
	            	...
	            	if (!haveInProgress) {
						...
						row += "<td>" + postFte + "</td>";
						row += "<td style='text-align:center'><button type='button' class='btn btn-primary' onclick='showPostDetails(\"" + postUid + "\")'>View</button><button type='button' name='btnRemovePost' class='btn btn-primary' onclick='removeFromPost($(this));enableEffectiveDate()'><i class='fa fa-times'></i> Remove</button></td>";
						row += "</tr></tbody>";
		...
		// Add to "To" Post
			...
			for (var m=0; m<$("#tblPositionFrom tbody").length; m++) {
				...
				if (tmpPostId == postId) {
					$("#divErrorMsg").html('Selected post cannot exist in Change From Post list.');
					$("#divError").show();
					$("#searchResultModel").modal("hide");
	            	hideLoading();
			...
			// Check is the post already exist
			var isExisting = false;
			for (var i=0; i<$("input[name='RequestProposedPostId']").length; i++) {
				if ($($("input[name='RequestProposedPostId']")[i]).val() == postId) {
					isExisting = true;
					break;
				}
			}
			...
			if (isExisting) {
				$("#divErrorMsg").html('Selected Post already exist.');
				$("#divError").show();
				$("#searchResultModel").modal("hide");
				return; 
			}
			...
			$.ajax({
	            ...
	            success: function(data) {
					...
					row += "<td style='text-align:center'><button type='button' name='btnEdit' class='btn btn-primary' onclick='editNewPosition(\""
							+ postSeq
							+ "\", this, " + rowNum + ")' style='margin-right:5px;'>View</button><button type='button' class='btn btn-primary' onclick='removeToPost($(this));$(\"#btnAddExistingPost\").prop(\"disabled\", false);$(\"#btnAddToPost\").prop(\"disabled\", false);enableEffectiveDate()'><i class='fa fa-times'></i> Remove</button>";
					//Position Details
	...
	$(function() {
		...
		if ("Y" != $("#canEditDetailInfo").val()) {
			...
			$("button[name='btnAddToPost']").prop("disabled", true);
			$("button[name='btnAddExistingPost']").prop("disabled", true); 
		...
		$("#ddHcmPostTitle").autocomplete({
			source: function(request, response) {
				$.getJSON("<%= request.getContextPath() %>/hcm/getHCMPostTitleList", request, function(result){
		...
		$("#editPostModel #txt_post_actual_start_date").attr("readonly", true);
	...
	function initNewPostModal(){
		...
		setEmpty('txt_limit_duration_end_date');
		...
		$("#editPostModel #txt_limit_duration_end_date").attr("disabled", true);
	...
	function performUpdate(postID, obj){
		...
		$("#tblPositionTo tbody tr[id='"+postID+"'] input[name='RequestPostActualEndDate']").val($('#editPostModel #txt_limit_duration_end_date').val());
		...
		if ($('#editPostModel #cbHoBuyServiceInd').prop("checked")) {
			$("#tblPositionTo tbody tr[id='"+postID+"'] input[name='RequestHoBuyServiceInd']").val("Y");
		}
		else {
			$("#tblPositionTo tbody tr[id='"+postID+"'] input[name='RequestHoBuyServiceInd']").val("N");
		}
		...
		// Remove All child
		var currentRow = $("#tblPositionTo tbody tr[id='"+postID+"'] div[name='hiddenTable']")[currentIdx];
		$(currentRow).empty();
		...
		for (var i=0; i<$("#tblPositionTo input[name='requestPostNoTo']").length; i++) {
			if ($($("#tblPositionTo input[name='requestPostNoTo']")[i]).val() == postID) {
				// Update the table
				var rank_desc ="";
				if ( $("#editPostModel #ddl_rank").val() != "") {
					rank_desc = $("#editPostModel #ddl_rank option:selected").text();
				...
				$("#tblPositionTo tr:eq(" + (i+1)+ ") td:eq(1)").text(rank_desc);
				$("#tblPositionTo tr:eq(" + (i+1)+ ") td:eq(2)").text(durationDesc);
				$("#tblPositionTo tr:eq(" + (i+1)+ ") td:eq(3)").text($("#editPostModel #postFTEValue").val());
			}
		}
		$("#editPostModel").modal("hide");
	}
	...
	function selectHcmPositionDetail(positionId, cluster, inst, dept, staffGroup, effectiveStartDate, name, fte, headcount, status, positionType) {
		...
		$.ajax({
            ...
            success: function(data) {
				...
				changeStaffGroup(data.deptCode, data.rankCode); 
				...
				// $("#frmDialog").bootstrapValidator('revalidateField', 'rank');
	...
	function clickDuration2() {
		...
		if ($("#editPostModel #rd_Duartion2").prop("checked") == false) {
			...
			$("#editPostModel #txt_limit_duration_end_date").val("");
			$("#editPostModel #txt_limit_duration_end_date").attr("disabled", true);
		}
		else {
			...
			$("#editPostModel #txt_limit_duration_end_date").attr("disabled", false);
		}
	...
	function changeStaffGroup(deptCode, rankCode) {
		...
		refreshDepartmentDropdown(deptCode, rankCode);
	}
	...
	function removeFromPost(row) {
		
		var table = row.parent().parent().parent().parent();
		var tmp = row.parent().parent();
		row.parent().parent().remove();
		if ($("#tblPositionFrom tbody tr").length == 0) {
			$("#tblPositionFrom tbody").html('<tr><td colspan="6">No record found.</td></tr>');
		}
	}
	
	function removeToPost(row) {
		
		var table = row.parent().parent().parent().parent();
		var tmp = row.parent();
		row.parent().parent().remove();
		if ($("#tblPositionTo tbody tr").length == 0) {
			$("#tblPositionTo tbody").html('<tr><td colspan="6">No record found.</td></tr>');
		}
	}
	...
	function refreshDepartmentDropdown(deptCode, rankCode) {
	...
	function refreshRankDropdown(rankCode) {
		var tmpRank = $("#editPostModel #ddl_rank").val();
		$.ajax ({
            url: "<%=request.getContextPath()%>/common/getRankByStaffGroup",
            type: "POST",
            data: {staffGroupCode: $("#editPostModel #ddl_staff_group").val(), deptCode: $("#editPostModel #ddl_department").val()},
            success: function(data) {
            	 ...
            	 if (typeof rankCode != "undefined") {
            	 	$("#editPostModel #ddl_rank").val(rankCode);
            	 }
            	 else {
...
<div id="page-content-wrapper">
	<div class="container-fluid">
		<div class="breadcrumbs">
			<a href="<c:url value="/home/home"/>"><i class="fa fa-home"></i>Home</a> > Request > Change of Staff Mix 
		</div>
		...
		<div class="section-title">
			<div class="title pull-left"><i class="fa fa-file-text-o"></i>Change of Staff Mix</div>
		</div>
		<form id="frmDetail" method="POST" enctype="multipart/form-data">
			...
			<div class="panel panel-custom-primary">
				<div class="panel-heading">
					<div class="panel-heading-title">Change From Post</div>
				</div>
				<div class="panel-body" style="padding-top:5px">
					...
					<table id="tblPositionFrom" class="table-bordered mprs_table">
						...
						<tbody>
							<c:forEach var="listValue"
								items="${formBean.requestPositionFromList}">
								<tr>
									<td><input type="hidden" name="requestPostNo"
										value="${listValue.postNo}">${listValue.postId}</td>
									...
									<td style='text-align: center'>
										...
										<button type="button" name="btnRemove" class="btn btn-primary" onclick="removeFromPost($(this));enableEffectiveDate()"><i class="fa fa-times"></i> Remove</button>
									</td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<div style="text-align: right">
						<button type="button" id="btnAddPost" name="btnAddPost" class="btn btn-primary" style="width:110px"
							onclick="startSearching()"><i class="fa fa-plus"></i> Add Post</button>
					</div>
				</div>
			</div>
			<br>
			<div class="panel panel-custom-primary">
				<div class="panel-heading">
					<div class="panel-heading-title">Change To Post</div>
				</div>
				<div class="panel-body" style="padding-top:5px">
						...
						<tbody>
							<c:forEach var="listValue" items="${formBean.requestPositionToList}" varStatus="pStatus1">
								<tr id="${listValue.requestPostId}">
									...
									<td>${listValue.postFTE} </td>
									<td style="text-align: center">
										<button type="button" name="btnEdit" class="btn btn-primary" onclick="editNewPosition('${ listValue.requestPostId }', this, ${pStatus1.index})">${listValue.isExistingPost == "Y"?"View":"Edit"}</button>	
										<button type="button" name="btnRemovePost" class="btn btn-primary" onclick="removeToPost($(this));enableEffectiveDate()"><i class="fa fa-times"></i> Remove</button>
					...
					<div style="text-align: right">
						...
						<button type="button" id="btnAddExistingToPost" name="btnAddExistingPost" class="btn btn-primary" style="width:160px" onclick="addExistingPosition()"><i class="fa fa-plus"></i> Add Existing Post</button>
					</div>
					
					<br>
					<div class="row">
						<div class="col-sm-2">
							<label for="txt_reason" class="field_request_label"><strong>Reason
									of Change</strong></label>
						</div>
						<div class="col-sm-9">
		...
		<div class="modal-dialog modal-dialog-custom" style="width:980px">
			<div class="modal-content">
				<div class="modal-header">
					<h4><b>Post Detail - Change to Post</b>
						...
						<div class="tab-content clearfix"
							...
							<div class="tab-pane active" id="tab0_basic_1">
								...
								<div class="row">
									<div class="form-group">
										<div class="col-sm-2">
											<label for="ddl_staff_group" class="field_request_label">Staff Group</label>
										</div>
										<div class="col-sm-3">
							...
							<div class="tab-pane" id="tab1_position_1">
									...
									<div class="col-sm-2 nopadding">
										<div class="form-inline form-group">
											...
											<form:input path="formBean.limitDurationNo" type="text"
												...
												name="txt_limit_duration_no" style="width:50px;" required="required" onchange="selectDuration()" oninput="this.value=this.value.replace(/[^0-9]/g,'');" maxlength="4"></form:input>
											<span class="form-space"></span>
											<form:select path="formBean.limitDurationUnit"
												id="ddl_limit_duration_unit" name="ddl_limit_duration_unit"
												class="form-control" style="width:60%;" required="required" onchange="selectDuration()">
												<form:option value="" label="- Select -" />
								...
								<div class="row clearfix">
									...
									<div class="col-sm-2 nopadding">
										<div class="form-inline">
											<form:radiobutton path="formBean.limitDurationType"
											id="rd_Duartion2" name="rd_Duartion" value="FIXED_END_DATE" onclick="clickDuration2()" required="required"></form:radiobutton>
											<span class="form-space"></span>
										<div class="form-group">
											<div class="input-group date" id="grpLimitDurationEndDate">
												<form:input path="formBean.limitDurationEndDate" type="text"
													class="form-control " id="txt_limit_duration_end_date"
													name="txt_limit_duration_end_date" required="required"  onchange="selectFixEndDate()"></form:input>
												<span class="input-group-addon"> <span
													class="glyphicon glyphicon-calendar"></span>
												</span>
											</div>
								...
								<div class="row">
									<div class="form-inline form-group">
										<div class="col-sm-2">
											<label for="postFTE" class="control-label">FTE<font class="star">*</font></label>
										</div>
										<div class="col-sm-6">
											<form:select path="formBean.postFTE" class="form-control"
												onchange="changeFTE2();" required="required">
												<form:option value="" label="- Select -" />
								...
								<div class="row">
										<div class="col-sm-12">
												...
												<div class="row">
													...
													<div class="col-sm-10">
														<form:textarea path="formBean.fundSrcRemark"
