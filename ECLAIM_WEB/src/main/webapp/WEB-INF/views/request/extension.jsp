...
<script>
	...
	function createNewRequest() {
		document.location.href = "<%= request.getContextPath() %>/request/extension";
	}
	
	function validateForm() {
		var errMsg = "";
	
		if ($("#tblExtension tbody tr td").html() == "No record found." || $("#tblExtension tbody tr").size() == 0) {
			errMsg += "Please select at least one post.<br/>";
		}
		
		for (var i=0; i< $("#tblExtension tbody tr").size(); i++) {
			if (isNaN($($("input[name='requestNewFTE']")[i]).val())) {
				errMsg += "FTE should be numeric.<br/>";
			}
			else {
				if ($($("input[name='requestNewFTE']")[i]).attr("disabled") != "disabled") {
		...
		if (canEditFinancialInfo == "Y") {
 		...
 		// if (daysBetween($("#requestEndDate").val(), $("input[name='postEndDate']").val()) <= 0) {
 		// 	errMsg += "Revised End Date should later than Original End Date.";
 		//}
	...
	function performDialogChecking() {
		...
		for (var i=0; i< numberOfFundingSource; i++) {
			var skipCheckingFromDC = false;
			if (i==0) {
				var oriAnnualPlan = $($("#tblFunding input[name$='oriAnnualPlanInd']")[i]).val();
				if (oriAnnualPlan == "") {
					skipCheckingFromDC = true;
				}	
			}
			
			alert(skipCheckingFromDC);
		...
		var totalFTE = $($($($("#tblExtension tbody tr")[currentIdx]).find("td")[3]).find("input")[0]).val();
	...
	function revalidateRequestEndDate(){
		// $("#frmDetail").bootstrapValidator('revalidateField', 'requestEndDate');
	}
	...
	function performSearch() {
		...
		$.ajax({
            url: "<%=request.getContextPath()%>/api/request/extension/searchPost",
			cache: false,
			type: "POST",
	...
	function selectPost(postUid, postId, rankDesc, postDurationDesc, fte, startDate, endDate, endDateDisplay) {				
		...
		for (var i=0; i<$("#tblExtension tbody tr").length; i++) {
			if ($($($("#tblExtension tbody tr")[i]).find("td")[0]).text().trim() == postId) {
				...
				if (!haveInProgress) {
            		...
            		$.ajax({
			            ...
			            success: function(dataPostDetail) {
							...
							if ($("#tblExtension tbody tr td").html() == "No record found.") {
								$("#tblExtension tbody").remove();
							}
							...
							if (fte < 1) {
								row += "<td style='text-align:center;width:60px'><input type='text' name='requestNewFTE' value='" + fte + "' class='form-control' style='width:60px' onchange='changeFTE()'/></td>";
							}
							else {
								row += "<td style='text-align:center;width:60px'><input type='text' name='requestNewFTE' value='" + fte + "' class='form-control' style='width:60px' onchange='changeFTE()' disabled/></td>";
							}
							...
							row += "<td style='text-align:center'><button id='btn_editPost_"
									...
									+ "', '" + dataPostDetail.limitDurationEndDate + "', this, " + rowNum + ")\" style='margin-right:5px;'>Edit Funding</button><button type='button' class='btn btn-primary' onclick='$(this).parent().parent().parent().remove();'><i class='fa fa-times'></i> Remove</button>";
							...
							row += "</td>";
							row += "</tr></tbody>";
							if ($("#tblExtension tbody").length == 0) {
								$("#tblExtension thead").after(row);
							}
							else {
								$("#tblExtension tbody:last").after(row);
							}
	...
	$(function() {
		...
		if ($("#tblExtension tbody tr").length == 0) {
			$("#tblExtension thead").after("<tbody><tr><td colspan='7'>No record found.</td></tr></tbody>");
		}
		...
		<c:choose>
			<c:when test="${formBean.canEditDetailInfo != 'Y'}">
				...
				$("#requestDurationValue").attr("disabled", true);
				$("#requestDurationUnit").attr("disabled", true);
	...
	function resumeSubmit() {
		...
		$.ajax({
            url: "<%=request.getContextPath() %>/request/validateExtension",
            type: "POST",
            data: {postUid: postUid, endDate: $("#requestEndDate").val(), value: $("#requestDurationValue").val(), unit: $("#requestDurationUnit").val() },
            success: function(data) {
            	if (data != "") {
            		hideLoading();
            		displayError(data);
	...
	function clickDurationExt() {
		if ($("#requestDuartion").prop("checked") == true || $("#requestDuartion2").prop("checked") == true) { 
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationType', false);
			$("#requestDuartion").attr("required", "");
			$("#requestDuartion2").attr("required", "");
		}/*
		else{
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationType', true);
		}*/
	
		if ($("#requestDuartion").prop("checked") == true) {
			$("#duration1Star").show();
			
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestEndDate', true);
			
			$("#requestEndDate").attr("disabled", false);
		}
		else {
			$("#duration1Star").hide();
			
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestEndDate', false);
		...
		if ($("#requestDuartion2").prop("checked") == true) {
			...
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationValue', true);
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationUnit', true);
			
			$("#requestDurationValue").attr("disabled", false);
			$("#requestDurationUnit").attr("disabled", false);
		}
		else {
			$("#duration2Star").hide();
			
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationValue', false);
			$("#frmDetail").data('bootstrapValidator').enableFieldValidators('requestDurationUnit', false);
			
			$("#requestDurationValue").attr("disabled", true);
			$("#requestDurationUnit").attr("disabled", true);
			$("#requestDurationValue").val("");
			$("#requestDurationUnit").val("");
		}
	}
...
<div id="page-content-wrapper">
	<div class="container-fluid">
		<div class="breadcrumbs">
			<a href="<c:url value="/home/home"/>"><i class="fa fa-home"></i>Home</a> > Request > Extension 
		</div>
		...
		<div class="section-title">
			<div class="title pull-left"><i class="fa fa-file-text-o"></i>Extension</div>
		</div>
		<form id="frmDetail" method="POST" enctype="multipart/form-data">
			...
			<div class="panel panel-custom-primary">
				<div class="panel-heading">
					<div class="panel-heading-title">Extension for Post</div>
				</div>
				<div class="panel-body" style="padding-top:5px">
					<table id="tblExtension" class="table-bordered mprs_table">
						<thead>
							<tr>
								<th>Post ID</th>
								<th>Rank</th>
						...
						<tbody>
							<c:forEach var="listValue" items="${formBean.requestPositionList}"  varStatus="pStatus1">
								<tr id="postNO_${listValue.postNo}">
									...
									<c:if test="${ listValue.postFTE != '1.0'}">
										<td style='text-align:center;width:60px'><input type="text" name="requestNewFTE" value="${listValue.postFTE}" class="form-control" style="width:60px" onchange='changeFTE()'/></td>
									</c:if>
									<c:if test="${listValue.postFTE == '1.0'}">
										<td style='text-align:center;width:60px'><input type="text" name="requestNewFTE" value="${listValue.postFTE}" class="form-control" style="width:60px" onchange='changeFTE()' disabled/></td>
									</c:if>
					...
					<br>
					<div class="row">
						<form:input path="formBean.requestStartDate" class="form-control" style="display:none"/>
						<div class="form-group">
							<div class="col-sm-2">
								<form:radiobutton path="formBean.requestDurationType"
										id="requestDuartion" name="rd_Duartion" value="FIXED_END_DATE" onclick="clickDurationExt()" required="required"></form:radiobutton>
								<label for="txt_reason" class="control-label"><strong>Revised End Date</strong><font id="duration1Star" class="star">*</font></label>
							</div>
							<div class="col-sm-2">
								<div class="input-group date">
								<form:input path="formBean.requestEndDate" class="form-control" required="required" onchange="revalidateRequestEndDate()"/>
					...
					<div class="row">
						<div class="form-group">
							<div class="col-sm-2">
								<form:radiobutton path="formBean.requestDurationType"
										id="requestDuartion2" name="rd_Duartion" value="DURATION_PERIOD" onclick="clickDurationExt()" required="required"></form:radiobutton>
								<label for="txt_reason" class="control-label"><strong>Revised Duration</strong><font id="duration2Star" class="star">*</font></label>
							</div>
							<div class="col-sm-1">
								<form:input path="formBean.requestDurationValue" class="form-control"
									oninput="this.value=this.value.replace(/[^0-9]/g,'');" required="required" maxlength="3"/>
							</div>
							<div class="col-sm-2">
								<form:select path="formBean.requestDurationUnit" class="form-control" style="width:60%;" required="required">
					...
					<div class="row">
						<div class="col-sm-2">
							<label for="txt_reason" class="field_request_label"><strong>Reason of Extension</strong></label>
						</div>
						<div class="col-sm-9">
