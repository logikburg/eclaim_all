...
<script>
	...
	function createNewRequest() {
		document.location.href = "<%= request.getContextPath() %>/request/changeOfFunding";
	}
	...
	function validateForm() {
		...
		if ($("#tblPositionChangeFunding tbody tr td").html() == "No record found." || $("#tblPositionChangeFunding tbody tr").size() == 0) {
			errMsg += "Please select at least one post.<br/>";
		}
		
		if (canEditFinancialInfo == "Y") {
			for (var i=0; i< $("#tblPositionChangeFunding tbody tr").size(); i++) {
		...
		if (!formEdited) {
			errMsg += "Funding information have no changes. Please update the detail funding information.";
		}
	...
	function editPost(mprspostNo, duration, postStartDate, postEndDate, obj, idx) {
		...
		if (duration == "Recurrent" || duration == "R") {
	...
	function savePostFunding() {
		...
		for (var i=0; i< numberOfFundingSource; i++) {
			...
			var hiddenTable = $($("div[name='hiddenTable']")[0]);
			if ($(hiddenTable.find("input[name$='annualPlanInd']")[i]).val() != annualPlanInd) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='programYear']")[i]).val() != programYear) {
				formEdited = true;
			} 
			else if ($(hiddenTable.find("input[name$='programCode']")[i]).val() != programCode) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='programName']")[i]).val() != programName) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='programTypeCode']")[i]).val() != programTypeCode) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='fundSrcId']")[i]).val() != fundSrcId) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='startDateStr']")[i]).val() != fundSrcStartDateStr) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='endDateStr']")[i]).val() != fundSrcEndDateStr) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='fundSrcFte']")[i]).val() != fundSrcFte) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='fundSrcRemark']")[i]).val() != fundSrcRemark) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='inst']")[i]).val() != inst) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='section']")[i]).val() != section) {
				formEdited = true;
			}
			else if ($(hiddenTable.find("input[name$='analytical']")[i]).val() != analytical) {
				formEdited = true;
			}
		}
	...
	function performSearch() {
		...
		$.ajax({
            url: "<%=request.getContextPath()%>/api/request/changeOfFunding/searchPost",
			cache: false,
			type: "POST",
            	...
            	if (data.postResponse != null) {
            		...
            		if (len > 0) {
	            		for (var i = 0; i < len; i++) {
							...
							html += "'" + data.postResponse[i].postFte  + "', '" + data.postResponse[i].postStartDateStr + "')\">" + data.postResponse[i].postId + "</a></td>"
	...
	function selectPost(postUid, postId, rankDesc, postDurationDesc, fte, postStartDate) {	
		// Check is Post ID exist 
		for (var i=0; i<$("#tblPositionChangeFunding tbody tr").length; i++) {
			if ($($($("#tblPositionChangeFunding tbody tr")[i]).find("td")[0]).text().trim() == postId) {
            	...
            	if (!haveInProgress) {
            		...
            		$.ajax({
			            ...
			            success: function(dataPostDetail) {
							...
							if ($("#tblPositionChangeFunding tbody tr td").html() == "No record found.") {
								$("#tblPositionChangeFunding tbody").remove();
							}
					
							var rowNum = $("#tblPositionChangeFunding tbody tr").length;
							...
							row += "<td name='requestPositionType' >" + (nvl(dataPostDetail.annual_plan_ind) == "Y" ? "Yes":"No") + "</td>";
							row += "<td name='requestDuration'>" + postDurationDesc + "</td>";
							row += "<td name='requestFte'>" + fte + "</td>";
							...
							row += "</tr></tbody>";
							if ($("#tblPositionChangeFunding tbody").length == 0) {
								$("#tblPositionChangeFunding thead").after(row);
							}
							else {
								$("#tblPositionChangeFunding tbody:last").after(row);
							}
	...
	function performDialogChecking() {
		...
		var totalFTE = $($($("#tblPositionChangeFunding tr")[currentIdx+1]).find("td")[4]).text();
	...
	$(function() {
		...
		if ($("#tblPositionChangeFunding tbody tr").length == 0) {
			$("#tblPositionChangeFunding thead").after("<tbody><tr><td colspan='6'>No record found.</td></tr></tbody>");
		}
		...
		<c:choose>
			<c:when test="${formBean.userHaveSaveRight != 'Y'}">
				...
				$("button[name='btnRemovePost']").attr("disabled", true);
...
<div id="page-content-wrapper">
	<div class="container-fluid">
		<div class="breadcrumbs">
			<a href="<c:url value="/home/home"/>"><i class="fa fa-home"></i>Home</a> > Request > Change of Funding 
		</div>
		...
		<div class="section-title">
			<div class="title pull-left"><i class="fa fa-file-text-o"></i>Change Of Funding</div>
		</div>
		<form id="frmDetail" method="POST" enctype="multipart/form-data">
			...
			<div class="panel panel-custom-primary">
				<div class="panel-heading">
					<div class="panel-heading-title">Change of Funding on Post</div>
				</div>
				<div class="panel-body" style="padding-top:5px">
					<table id="tblPositionChangeFunding" class="table-bordered mprs_table">
						<thead>
							<tr>
								<th>Post ID</th>
								<th>Rank</th>
							...
							<c:forEach var="listValue" items="${formBean.requestPositionList}"  varStatus="pStatus1">
								<tr id="postNO_${ listValue.postNo }">
									<td><input type="hidden" name="requestPostNo" 
										value="${listValue.postNo}">${listValue.postId}
									</td>
...
<!--  MPRSPostFundingResultModel - Start -->
<form id="frmDialog">
